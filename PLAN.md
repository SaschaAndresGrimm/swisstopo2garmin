...
| **Routing (ActiveRouting equivalent)** | **implemented, unverified on hardware** | NET and NOD via mkgmap `--route` on both passes, `road_class`/`road_speed` on the road types, and access, oneway, bridge, tunnel and surface derived from `verkehrsbeschraenkung`, `befahrbarkeit`, `richtungsgetrennt`, `kunstbaute`, `belagsart` and `stufe`. Possible without touching the cartography because the roads already used standard Garmin types — an extended type cannot be a road. The one dangerous assumption, that a directionally separated carriageway is digitised the way traffic moves, is **verified against the data**: motorway ramps agree 100 % of the time in the cases clear enough to judge (docs/routing.md). Costs a measured 33.7 B per transport feature, ~12 % of a typical map, and the estimate accounts for it. **Off by default** and labelled unverified in the app until §C.7 of the device sweep is done. |
| **Raster / paper-map view** | **implemented, unverified on hardware** | Garmin Custom Maps: JPEG tiles from `ch.swisstopo.pixelkarte-farbe` in a KMZ, installed to `Garmin/CustomMaps` (SPEC.md §8.5, FR-R1…FR-R6). The deferral reasoning was that the caps make it not worth having; the caps are real (100 tiles of 1 MP, community-sourced) but the arithmetic says they cover **12 × 12 km at the source's native 1.25 m/px** — a day's walking at full sharpness. Measured, not assumed: 0.201 B/px over farmland to 0.528 over central Basel, so a full overlay is ~55 MB against a budget in gigabytes — the tile count binds, the bytes do not. Needed a STORE-only ZIP writer (`zip.rs` is the read side); no new dependency, `flate2` already exposes CRC-32. Two silent-failure modes were checked against the live service rather than assumed: WMS 1.3.0 axis order (EPSG:4326 is lat-first, EPSG:2056 easting-first — the wrong order returns a blank tile with HTTP 200, and a 6.4 kB blank was briefly mistaken for a working request during development) and error responses (a rejected GetMap is 200 with an XML ServiceException, so the JPEG magic bytes are checked). The grid is planned in degrees, not LV95, because a `GroundOverlay` is an axis-aligned `LatLonBox` and LV95 is oblique Mercator. Output verified as a file with Python's `zipfile`: all CRCs match, the KML parses, every href resolves. **Off by default.** What is entirely unverified is whether a device *draws* it — SPEC.md §1.4 spent the project believing Edge and fēnix could not, and both profiles advertising `Garmin/CustomMaps` is evidence, not proof. See docs/raster.md and §C.9 of the device sweep, whose first question is whether the device lists the map at all; a "no" there retires the feature. **Open question for the next device session (§C.9.8):** does a custom map *disabled* in the map manager still count toward the 100-tile total? If it does not, a user can carry one KMZ per region and enable the one they need, which is the only route to anything like national coverage — the tile budget is a pixel budget, so neither a coarser pyramid nor a bigger file buys coverage (docs/raster.md). |
| **Address search** | **implemented, unverified on hardware** | swissTLM3D has 173,860 street *names* and no house numbers, so this needed a dataset the project did not have: `ch.swisstopo.amtliches-gebaeudeadressverzeichnis`, the official register, 3.3 M addresses as a 137 MB zipped CSV under the same OGD terms. It is not in the first page of the STAC catalogue and a search of that page came back empty — paging all 512 collections found it. Streamed a line at a time and clipped to the area: 3.3 M rows in 1.5 s at constant memory, since holding the file is 468 MB (NFR-2). Emitted as POIs with `addr:*` plus `mkgmap:city`/`mkgmap:postal_code`, matched to roads by `--housenumbers` against the `mkgmap:street` the style now sets. Verified on disk: NET goes from 0 to 44,140 B and MDR grows. Costs a measured 10.5 B per address. **Off by default.** See docs/addresses.md. |
| **Write to the device over MTP** | **not started, investigated** | Recent Edge models are MTP-only: the Edge 840 has no USB-mode setting at all (Garmin's own Edge 840 forum), so it never appears under `/Volumes` and the app cannot write to it directly on macOS. Windows needs nothing — File Explorer speaks MTP natively — and on Linux a user can mount it with `jmtpfs` or `gio mount`, after which the existing mass-storage path works unchanged. **So this is a macOS-only gap.** The route that fits this project is the one it already uses for mkgmap and splitter: shell out to a CLI across a process boundary, no FFI, so `unsafe_code = "forbid"` stands. The concrete candidate is `aft-mtp-cli` from [android-file-transfer-linux](https://github.com/whoozle/android-file-transfer-linux) (LGPL-2.1, compatible) — which is exactly what OpenMTP bundles, verified by reading its own bundle: `OpenMTP.app/Contents/Resources/bin/mtp-cli` is that binary renamed, and its strings still say "android file transfer for linux version". Costs: another per-platform vendored binary in a bundle that is already ~140 MB, and the copy in OpenMTP is x86_64-only so an arm64 build would be needed. One trap found while investigating: **OpenMTP holds the USB device exclusively** (`UsbExclusiveOwner = "pid …, OpenMTP Helper"` in `ioreg`, which is visible in this project's own test fixture), so any MTP support must detect that and say "quit OpenMTP first" rather than failing obscurely. Until then the export-to-folder escape hatch plus a manual copy is the supported path, and the UI now says so per device. |
| **SAC hut details (scraping)** | **not started, and recommended against** | The licence question was the gate, and it has been asked: **no grant of reuse was found.** `robots.txt` permits general crawling (`User-Agent: * / Allow: /`, with disallows for search, forms, contact and favourites pages — none covering hut pages), so automated *access* is not prohibited. But sac-cas.ch publishes an Impressum and booking AGBs, not an open-data licence, and **absence of a prohibition is not a grant**: redistributing SAC's compiled hut details inside a GPL-3.0 map would assert a right nobody gave. The value is also thin — the map already carries 506 named huts, and a phone number on a watch screen is not what anybody reaches for. **The right next step is an email, not a scraper:** SAC is a members' club that wants its huts found and may well say yes in writing. If they do, `sactid` is a stable key and a small cache refreshed rarely would be enough. |
...
| Label language (FR-53) | **done** | From swissNAMES3D, which tags names by language; swissTLM3D's pipe-separated names have no language order and cannot answer it. |
...
panel takes the same area to 958,464 B.
