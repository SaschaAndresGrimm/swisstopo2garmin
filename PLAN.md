...
| **SAC hut details (scraping)** | **not started** | The map carries 506 named huts; contact details, opening hours and warden status live only on the SAC portal, which `unterkuenfte-winter` links to but does not contain. Fetching them means scraping sac-cas.ch, so it needs a licensing check first — whether their terms permit it, and whether the result can be redistributed in a GPL-3.0 map. If they do: the `sactid` column is a stable key, so a small cache keyed on it, refreshed rarely, would be enough. Do not start the scraper before the licence question is answered. |
